# train.py
# Version 3.2 - Replaced multithreading with multiprocessing and implemented robust process cancellation
...
def train_model(data, labels, vocab_size, n_iterations, step_size, temp, num_epochs, input_dim_bounds, hidden_dim_bounds, learning_rate_bounds, save_path, device):
    ...
    #def quantum_inspired_optimization(objective, bounds, n_iter, initial_step_size, temp, data, labels):
        ...
        step_size_min = initial_step_size * 0.1
        step_size_max = initial_step_size * 10
        ...
        if device.type == 'cuda':
            for i in range(n_iter):
                ...
                try:
                    ...
                    print(f"Improvement rate: {(scores[0] - best_eval) / (scores[0] + 1e-10):.6f}")
            ...
            def evaluate_process(candidate, i, return_dict, progress, early_termination):
                try:
                    print(f"Process {i} started for candidate: {candidate}")
                    result = objective(candidate, data, labels, progress, i)
                    print(f"Process {i} completed with result: {result}")
                    return_dict[i] = result
                    print(f"Result for process {i} stored in return_dict")
                except Exception as e:
                    print(f"Error in process {i}: {e}")
                    return_dict[i] = float('inf')
                    import traceback
                    traceback.print_exc()
            ...
            #with Pool(processes=cpu_count()) as pool:
                ...
                for i in range(n_iter):
                    ...
                    print(f"Submitting candidate {i+1}/{n_iter} for evaluation: {candidate}")
                    process = pool.apply_async(evaluate_process, (candidate, i, return_dict, progress, early_termination))
                    processes.append(process)

                for process in processes:
                    process.wait()  # Ensure each process completes

                print("All subprocesses have completed. Gathering results...")

                for key in return_dict.keys():
                    print(f"Result from process {key}: {return_dict[key]}")
    ...
    def quantum_inspired_optimization(objective, bounds, n_iter, initial_step_size, temp, data, labels):
        ...
        if device.type == 'cuda':
            ...
            with Pool(processes=cpu_count()) as pool:
                ...
                update_thread = threading.Thread(target=update_pbar, args=(processes,))
                update_thread.start()
