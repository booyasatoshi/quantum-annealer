# train.py
# Version 2.6 - Implemented immediate thread stopping, future cancellation, and result collection at early termination
...
def train_model(data, labels, vocab_size, n_iterations, step_size, temp, num_epochs, input_dim_bounds, hidden_dim_bounds, learning_rate_bounds, save_path, device):
    ...
    def quantum_inspired_optimization(objective, bounds, n_iter, initial_step_size, temp, data, labels):
        ...
        if device.type == 'cuda':
            ...
            for i in range(n_iter):
                if early_termination:
                    print("Early termination triggered. Stopping optimization.")
                    break
            ...
            def evaluate_thread(candidate, i):
                nonlocal best, best_eval, curr, curr_eval, worse_accepted, significant_jumps, step_size, early_termination, early_termination_state
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_count()) as executor:
                ...
                for i in range(n_iter):
                    ...
                    candidate = curr + step_size * np.random.randn(bounds.shape[0])
                    candidate = np.clip(candidate, bounds[:, 0], bounds[:, 1])
                    futures.append(executor.submit(evaluate_thread, candidate, i))
                    
                    # Update main progress bar
                    pbar.update(1)
                    pbar.refresh()
                
                # Wait for ongoing futures to complete and cancel pending ones
                for future in concurrent.futures.as_completed(futures):
                    if early_termination_event.is_set():
                        ...
                        for f in futures:
                            f.cancel()
                        break
                    future.result()  # This will raise an exception if the future was cancelled
        ...
        final_state = early_termination_state if early_termination else {
            'iteration': n_iter,
            'best': best,
            'best_eval': best_eval,
            'scores': scores,
        ...
        return best, best_eval, scores, early_termination, final_state
